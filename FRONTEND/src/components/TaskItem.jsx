...
const TaskItem = ({ task, onRefresh, onLogout, showCompleteCheckbox = true }) => {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${API_BASE}/${task._id}/Task`, { withCredentials: true });
      onRefresh?.();
    } catch (err) {
      console.error(err);
      if (err.response?.status === 401) onLogout?.();
  ...
  const handleSave = async (updatedTask) => {
    try {
      ...
      await axios.put(`${API_BASE}/${task._id}/Task`, payload, { withCredentials: true });
      ...
      <div className={`${TI_CLASSES.wrapper} ${borderColor}`}>
            ...
            {subtasks.length > 0 && (
              <div className={TI_CLASSES.subtasksContainer}>
                ...
                <div className={TI_CLASSES.progressBarBg}>
                  <div
                    className="h-1.5 bg-gradient-to-r from-[#6A0DAD] to-[#FF6EC7] rounded-full animate-pulse"
                    style={{ width: `${progress}%` }}
                  />
                </div>
                <div className="space-y-1 sm:space-y-2 pt-1">
                  {subtasks.map((st) => (
                    <div key={st._id || st.title} className="flex items-center gap-2 group/subtask">
                      <input
                        type="checkbox"
                        ...
                        onChange={() =>
                          setSubtasks((prev) =>
                            prev.map((s) =>
                              s._id === st._id ? { ...s, completed: !s.completed } : s
                            )
                          )
                        }
        ...
        <div className={TI_CLASSES.rightContainer}>
          <div className="relative">
            ...
            {showMenu && (
              <div className={`${TI_CLASSES.menuDropdown} bg-[#1A1A2E] border border-[#6A0DAD]/30 rounded-md shadow-lg`}>
                {MENU_OPTIONS.map((opt) => (
                  <button
                    key={opt.action}
                    onClick={() => handleAction(opt.action)}
                    className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm hover:bg-[#6A0DAD]/10 hover:text-[#FF6EC7] flex items-center gap-2 transition-colors duration-200"
                  >
                    {opt.icon}
                    {opt.label}
                  </button>
